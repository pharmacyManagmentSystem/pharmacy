...
class _ManagePharmacistPageState extends State<ManagePharmacistPage> {
  ...
  void applyFilter() {
    if (searchQuery.isEmpty) {
      ...
      filteredPharmacists = allPharmacists
          .where((pharmacist) =>
          pharmacist["email"].toLowerCase().contains(searchQuery.toLowerCase()))
          .toList();
    }
  }
  ...
  void _showPharmacistDialog({Map<String, dynamic>? pharmacist}) {
    ...
    final emailController = TextEditingController(text: pharmacist?["email"] ?? "");
    final phoneController = TextEditingController(text: pharmacist?["phone"] ?? "");
    final addressController =
    TextEditingController(text: pharmacist?["pharmacy_address"] ?? "");
    final passwordController =
    pharmacist == null ? TextEditingController() : null; // only for new pharmacist
    ...
    Future<void> savePharmacist() async {
      ...
      if (pharmacist == null) {
        ...
        // UPDATE EXISTING PHARMACIST WITHOUT CHANGING PASSWORD
