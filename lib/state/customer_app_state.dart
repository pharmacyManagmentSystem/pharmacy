  ...
  Future<bool> addProductToCart(Product product,
    ...
    try {
      ...
      if (productSnapshot.exists && productSnapshot.value is Map) {
        ...
        // Calculate total quantity from batches if available
        int availableQuantity = 0;
        if (productData['batches'] != null && productData['batches'] is Map) {
          final batches = productData['batches'] as Map;
          ...
          // Fallback to quantity field (legacy)
          availableQuantity = productData['quantity'] is num
  ...
  Future<String> submitOrder({
    ...
    for (final entry in _cart.entries) {
      ...
      if (batches != null && batches.isNotEmpty) {
        ...
        for (final entry in sortedBatchIds) {
          if (remainingToDeduct <= 0) break;

          final batchQty = entry.value['quantity'] is num
              ? (entry.value['quantity'] as num).toInt()
              : int.tryParse(entry.value['quantity']?.toString() ?? '0') ?? 0;

          if (batchQty > 0) {
            final deductQty =
                remainingToDeduct > batchQty ? batchQty : remainingToDeduct;
            final newBatchQty = batchQty - deductQty;

            if (newBatchQty <= 0) {
              batches.remove(entry.key);
            } else {
              batches[entry.key] = {
                ...entry.value,
    ...
    final orderMap = {
      ...
      'acceptStatus':
          'pending', // Status for pharmacist: pending, accepted, rejected
      'deliveryStatus':
          null, // Status for delivery person: out_for_delivery, delivered
