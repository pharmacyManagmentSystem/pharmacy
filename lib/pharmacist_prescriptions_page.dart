...
class PharmacistPrescriptionsPage extends StatelessWidget {
      ...
      body: StreamBuilder<DatabaseEvent>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return const Center(child: Text('Unable to load pending prescriptions.'));
          }
          ...
          if (raw is Map) {
            raw.forEach((k, v) {
              if (v is Map) items.add(MapEntry(k.toString(), Map<dynamic, dynamic>.from(v)));
            });
          }

          if (items.isEmpty) return const Center(child: Text('No prescriptions waiting for review.'));
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final productName = data['name']?.toString() ?? data['productName']?.toString() ?? 'Product';
              final customer = data['customerId']?.toString() ?? data['customerName']?.toString() ?? 'Customer';
              final qty = data['quantity']?.toString() ?? '1';
              final url = data['imageUrl']?.toString() ?? data['prescriptionUrl']?.toString() ?? '';
                              ...
                              children: [
                                SizedBox(
                                  ...
                                  child: ElevatedButton.icon(
                                    onPressed: () => _approve(context, id, data),
  ...
  Future<void> _approve(BuildContext context, String id, Map<dynamic, dynamic> data) async {
    final customerId = data['customerId']?.toString();
    final pharmacyId = data['ownerId']?.toString() ?? '';
    try {
      // mark customer's cart entry as approved (pendingApproval=false)
      if (customerId != null && customerId.isNotEmpty) {
        final customerCartRef = DatabaseService.instance.customerCartRef(customerId).child(id);
        ...
        // Update the cart item status with all necessary fields
        ...
        // notify customer
        final notifRef = DatabaseService.instance.customerNotificationsRef(customerId).push();
        await notifRef.set({
      ...
      await DatabaseService.instance.pendingPrescriptionsRef(pharmacyId).child(id).remove();
  ...
  Future<void> _reject(BuildContext context, String id, Map<dynamic, dynamic> data) async {
    final customerId = data['customerId']?.toString();
    final pharmacyId = data['ownerId']?.toString() ?? '';
    try {
      if (customerId != null && customerId.isNotEmpty) {
        await DatabaseService.instance.customerCartRef(customerId).child(id).update({
          'pendingApproval': false,
          'rejected': true,
